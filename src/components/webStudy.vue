<template>
  <div class="webStudy">
    ...
    <div class="webStudy-left" style="float: left;">
      <p class="crumbs-main">
        <i class="el-icon-location"></i>
        <span>您当前的位置:</span>
        <el-breadcrumb separator-class="el-icon-arrow-right">
          <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
          <el-breadcrumb-item>{{this.$route.params.formData}}</el-breadcrumb-item>
        </el-breadcrumb>
      </p>
        <ul class="webStudy-main-left" v-if="this.webContentOne">
          <router-link :to="{name:'pageContent', params:{id: item.id}}" tag="li" v-for="item in this.webContentOne">
            <img :src="item.filePath">
            <div class="webStudy-main-left-content">
              <h2>{{item.title}}</h2>
              <p>
                <span>作者：{{item.author}}</span>
                <span>时间：{{item.time}}</span>
                <span>分类：<router-link :to="{name:'webStudy', params:{ formData: item.name } }">{{item.name}}</router-link></span>
              </p>
              <div class="webStudy-main-left-maincontent">{{item.abstract}}</div>
            </div>
          </router-link>
        </ul>
        <ul class="webStudy-main-left-more" v-if="this.webContentOne.length == 0">
          <h2>没有更多文章</h2>
        </ul>
        <el-pagination
...
export default {
  ...
  watch: {
    '$route': 'getInitwebStudy'
  },
  created () {
    this.getInitwebStudy()
  },
  methods: {
    getInitwebStudy () {
      ...
      this.$http.post('/api/headerTitle/webstudy', {formData: this.formData}).then((res) => {
          ...
          span
            ...
            &:last-child
              &:hover
                color: red
                text-decoration: underline
        .webStudy-main-left-maincontent
          font-size: 13px
          height: 80px
          margin-top: 5px
          line-height: 28px
